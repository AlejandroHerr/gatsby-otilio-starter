...
export default class NProgress extends PureComponent {
  ...
  componentDidUpdate() {
    ...
    const { isPlaying, progress, isReseting } = this.state;

    /* if (progress >= 100 && !isReseting) {
      return this.setState({ isReseting: true, isPlaying: false });
    }

    if (isReseting) {
      console.log('was reseting');
      return this.setState(
        { progress: 0 },
        () => setTimeout(() => this.setState({ isReseting: false, progress: 0 }), speed),
      );
    }
    */
    if (progress === 0 && isReseting) {
      setTimeout(() => this.setState({ isReseting: false }), 1);
    }

    if (isPlaying && progress < 100 && !isReseting) {
      return setTimeout(() => {
        this.increment();
      }, speed);
    }
  ...
  onTransitionEnd() {
    const { progress, isReseting } = this.state;
    console.log('onTransitionEnd', progress, isReseting);
    if (progress >= 100 && !isReseting) {
      return this.setState({ isReseting: true, isPlaying: false }, () => {
        this.setState({ progress: 0 });// , () => {
        //  this.setState({ isReseting: false, progress: 0 });
        // });
      });
    }

    // if (progress === 0 && isReseting) {
    // console.log('reseting and 0');
